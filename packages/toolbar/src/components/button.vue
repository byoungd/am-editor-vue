...
import ATooltip from './ToolTip.vue'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const hotkey = ref<string | boolean | undefined>(false)
    if (props.hotkey) {
      hotkey.value = props.hotkey.toString()
    }
    //默认获取插件的热键
    if (props.engine && (hotkey.value === true || hotkey.value === undefined)) {
      hotkey.value = autoGetHotkey(
        props.engine,
        props.command && !Array.isArray(props.command) ? props.command.name : props.name
      )
    }
    if (typeof hotkey.value === 'string' && hotkey.value !== '') {
      hotkey.value = formatHotkey(hotkey.value)
    }
